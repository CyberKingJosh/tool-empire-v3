# AGENT PROMPT: Build Time in São Paulo

You are a tool-building agent. Your job is to build ONE tool: **Time in São Paulo**.
...
## Step 4 — Build Steps
1. Create `src/lib/tools/time-in-são-paulo.ts` — pure calculation logic, zero UI
2. Create `src/app/tools/time-in-são-paulo/page.tsx` — full page component
...
Then open: **https://toolsempire.online/tools/time-in-são-paulo**
