---
You are building Tool #155: Prompt Token Counter
...
6. /tool-empire-v3/tools/155-prompt-token-counter/SPEC.md — your specific tool data

TOOL SUMMARY:
Name: Prompt Token Counter
Target keyword: prompt token counter all models
...
API Cost: FREE
Beat difficulty: N/A
Free tier: Full prompt token counter with instant results, sharing, and copy
$1 AUD export: No
Pro tier: N/A
...
Search "prompt token counter all models". Open top 3 results.
...
Step 1: Create /tool-empire-v3/src/app/tools/prompt-token-counter/page.tsx
...
Step 2: Create /tool-empire-v3/src/lib/tools/prompt-token-counter.ts
...
/tool-empire-v3/src/app/tools/prompt-token-counter/api/route.ts
Zod validation. Rate limited. No client-exposed errors.

Step 8: SEO audit
Title: 'Prompt Token Counter Free Online | [Domain]'
H1: exact target keyword match only
Meta: 'Free prompt token counter. [one sentence]. No signup. Instant results.'
...
Update /tool-empire-v3/tools/155-prompt-token-counter/STATUS.md: NOT_STARTED → BUILT
Update /tool-empire-v3/BUILD_STATUS.md: add tool with status BUILT
Final BUILD_LOG.md entry: "All 12 steps complete. Prompt Token Counter is BUILT."

CONTEXT RECOVERY RULE:
If interrupted: open new Claude Code session. Say:
"Read BUILD_LOG.md in /tool-empire-v3/tools/155-prompt-token-counter/ and resume from where it stopped."
...
Then open: **https://toolsempire.online/tools/prompt-token-counter**
